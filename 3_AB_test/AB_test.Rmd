...
After seeing an simple A / B test example, we'll take a step back and see how do come up with a well-defined metric.

**Step1: Generate a high level metrices** 

A non-exhaustive list of techniques that might be able to help you come up with high level metrices that you might want to test out in your experiment.
...
**Step2: Turning it into a well-defined metrices** 

Recall that in the examples above, we've chosen the high-level metric click-through probability. And this is defined by the unique number of people how click the button versus the number of users who visited the home page.
...
**Step3: Segmenting and filtering your traffic**
...
For example, you're looking at your metric of total active cookie users over time. And you see a spike in one of the timelines. After confirming that this is not caused by seasonal variation, we can look at different segment of our visitors to see if one of the segment is causing the spike. 



Suppose we have chosen segment to be platforms ( mobile and desktop ). Plot the rate and probability by platforms to determine 

## Others

Some principles to consider when designing A / B test :

1. Risk : What risk is the participant undertaking? The main threshold is whether the risk exceeds that of “minimal risk”. e.g. people's health.
2. Benefits : What benefits might result from the study?
3. Alternatives : What other choices do people have? In a sense are the people coerced to participate since they lack other alternatives to choose from.
4. Data Sensitivity : What level of confidentiality can participants expect? Studies without personal data like names or simply data that can't be match to an individual are generally fine. Though a terms of service or a privacy policy is necessary, as it contains a clear statement of what data are you going to collect and how you use it.
