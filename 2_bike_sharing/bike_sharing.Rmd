...
We'll combine the rest of the  feature engineering steps into one single  [`ExtractFeatures`][ExtractFeatures] function. Note that the actual test set does not include the `casual` and `registered` column. Hence, for this function, we also defined a data.type parameter so that we can use it for both training and test set.
...
Now that we have preprocessed our dataset, we're ready to apply the machine learning algorithms.
