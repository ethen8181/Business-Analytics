...
source("/Users/ethen/Business-Analytics/2_marketing_analysis/code/segmentation.R")
...
# average revenue that you would expect from each segment's segment in the next year 
revenue_average <- revenue2[ , .( avg_revenue = mean(revenue) ), by = segment ] %>%
				   arrange( desc(avg_revenue) )

# expected total revenue generated by each segment 
revenue_total <- revenue2[ , .( total_revenue = sum(revenue) ), by = segment ] %>%
...
# visualize 
ggplot( revenue_average, aes( reorder( segment, -avg_revenue ), avg_revenue ) ) + 
